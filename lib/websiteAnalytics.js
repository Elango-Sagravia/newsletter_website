  ...
  if (allowedDomains.size > 0 && process.env.LINKLY_API_KEY) {
    try {
      ...
      if (!listRes.ok) {
        ...
        console.error("Linkly list_links failed:", {
      ...
      if (!contentType.includes("application/json")) {
        ...
        console.error("Linkly list_links returned non-JSON:", text);

        throw new Error("Linkly list_links returned non-JSON response");
      }
